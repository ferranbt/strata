...
mod tests {
    ...
    /// Serve `dummy` over the wire and drive it with a generated gRPC client: the
    /// endpoint listing crosses, then a read streams its schema followed by pages
    /// whose Arrow IPC decodes back to the rows the generator produced.
    #[tokio::test]
    async fn serves_a_provider_over_grpc() -> Result<()> {
        ...
        // Nothing is usable before the host mounts it with a config.
        assert_eq!(
            client
        ...
        let listed = client.endpoints(EndpointsRequest { path: None }).await?.into_inner();
        ...
        let declared: schema::Schema = serde_json::from_str(&declared)?;
        assert_eq!(declared, row_schema);
        ...
        while let Some(message) = stream.message().await? {
            ...
            let page = decode_page(page)?;
            rows += page.data.row_count();
            pages += 1;
        }
    ...
    /// Mount an out-of-process provider into a `Registry` and drive it through the
    /// ordinary `ProviderObject` surface: write a dataset, then read it back. The
    /// registry never learns it isn't local.
    #[tokio::test]
    async fn registry_mounts_a_remote_provider() -> Result<()> {
        use crate::providers::sql::WriteResult;
        ...
        // The endpoint listing survived the crossing, metadata included.
        let tables = provider
            .endpoints()
            .into_iter()
            .find(|e| e.path == "/tables/:table" && e.method == Method::List)
            .expect("the table read endpoint");
        assert!(tables.metadata.queryable);
        assert_eq!(tables.metadata.strategy, Some(ListStrategy::Offset));
        ...
        let written = provider
            ...
            .await?;
        let written: WriteResult = serde_json::from_value(written.output)?;
